---
title: "Get a mapping table of orthologs between mouse and human"
...
```{r}
set.seed(1234)
OUTDIR <- "./00_geneannotation_output";
...
We get the full gene annotation for mouse powered by EnsEMBL
```{r}
mouseDB <- getBM(attributes=c("ensembl_gene_id", "mgi_symbol"), 
	       mart=mouse, uniqueRows=TRUE)
```

Then we focused on those that present orthologs
```{r}
...
```{r}
# ensembl-symbol
saveRDS(mouseDB, file=paste0(OUTDIR, "/mouse.rds"))
saveRDS(orth.mouse, file=paste0(OUTDIR,"/orthmouse.rds"))
